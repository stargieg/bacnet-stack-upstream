...
static void
Analog_Output_Reset_Event_Properties(uint32_t object_instance)
{
    unsigned j;
    struct object_data *pObject;
 ...
 *   BACNET_STATUS_ERROR for ERROR_CODE_INVALID_ARRAY_INDEX
 */
static int Analog_Output_Event_Message_Texts_Encode(
    uint32_t object_instance, BACNET_ARRAY_INDEX index, uint8_t *apdu)
{
    ...
    text = Analog_Output_Event_Message_Text(object_instance, index);
    if (text) {
...
int Analog_Output_Read_Property(BACNET_READ_PROPERTY_DATA *rpdata)
{
    ...
    switch (rpdata->object_property) {
        ...
        case PROP_EVENT_MESSAGE_TEXTS:
            apdu_len = bacnet_array_encode(
                rpdata->object_instance, rpdata->array_index,
                Analog_Output_Event_Message_Texts_Encode,
                MAX_BACNET_EVENT_TRANSITION, apdu, apdu_size);
            if (apdu_len == BACNET_STATUS_ABORT) {
                rpdata->error_code =
                    ERROR_CODE_ABORT_SEGMENTATION_NOT_SUPPORTED;
...
bool Analog_Output_Write_Property(BACNET_WRITE_PROPERTY_DATA *wp_data)
{
    ...
    switch (wp_data->object_property) {
        case PROP_PRESENT_VALUE:
            ...
            if (status) {
                if (Analog_Output_Present_Value_Write(
...
#if defined(INTRINSIC_REPORTING)
static const char *Analog_Output_Event_Message(
    uint32_t object_instance,
    ...
    uint32_t object_instance)
{
    ...
    if (pObject->Ack_notify_data.bSendAckNotify) {
        ...
        debug_printf(
            "Analog-Output[%d]: Send Acknotification.\n", object_instance);
        ...
        if (FromState != ToState ||
            ...
            switch (ToState) {
                case EVENT_STATE_HIGH_LIMIT:
                    ...
                    msgText = Analog_Output_Event_Message(
            ...
            debug_printf(
                "Analog-Output[%d]: Event_State goes from %s to %s.\n",
    ...
    if (SendNotify) {
        ...
        debug_printf(
            "Analog-Output[%d]: Notification Class[%d]-%s "
            "%u/%u/%u-%u:%u:%u.%u!\n",
        ...
        if ((event_data.notifyType != NOTIFY_ACK_NOTIFICATION) &&
            ...
            debug_printf("Analog-Output[%d]: Ack Required!\n", object_instance);
    ...
    BACNET_GET_EVENT_INFORMATION_DATA * getevent_data)
{
    ...
    pObject = Keylist_Data(Object_List, Analog_Output_Index_To_Instance(index));
...
#if defined(INTRINSIC_REPORTING)
            Analog_Output_Reset_Event_Properties(object_instance);
#endif
        } else {
            return BACNET_MAX_INSTANCE;
        }
