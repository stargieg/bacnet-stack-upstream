...
/**
* Analog_Output_Object() replaced by
* Keylist_Data(Object_List, object_instance)
*
* Analog_Output_Object_Index() replaced by
* Keylist_Data(Object_List, Analog_Output_Index_To_Instance(index)
*
*/
/**
 ...
 */
static struct object_data *Analog_Output_Object(uint32_t object_instance)
{
 ...
 * @return object found in the list, or NULL if not found
 */
static struct object_data *Analog_Output_Object_Index(int index)
{
...
int Analog_Output_Read_Property(BACNET_READ_PROPERTY_DATA *rpdata)
{
    ...
    switch ((int)rpdata->object_property) {
        ...
        case PROP_FEEDBACK_VALUE:
            real_value = pObject->Feedback_Value;
            apdu_len = encode_application_real(&apdu[0], real_value);
            break;
#endif
    ...
    BACNET_GET_EVENT_INFORMATION_DATA * getevent_data)
{
    ...
    instance = Analog_Output_Index_To_Instance(index);
	...
	struct itr_ctx *ictx)
{
    ...
    if (!pObject->Object_Name) {
        snprintf(options, sizeof(options), "Analog Output %i", idx);
...
#if defined(INTRINSIC_REPORTING)
    pObject->Feedback_Value = ucix_get_option_float(ictx->ctx, ictx->section, sec_idx,
        "fb_value", 0);
    pObject->Event_State = EVENT_STATE_NORMAL;
    /* notification class not connected */
