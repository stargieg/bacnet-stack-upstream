 ...
 */
static struct object_data *Analog_Input_Object(uint32_t object_instance)
{
 ...
 * @return object found in the list, or NULL if not found
 */
static struct object_data *Analog_Input_Object_Index(int index)
{
    ...
    switch (wp_data->object_property) {
        ...
        case PROP_COV_INCREMENT:
            ...
            if (status) {
                ...
                value.type.Real);
                ucix_add_option_float(
                    ctxw, sec, idx_c, "cov_increment",
        ...
        case PROP_HIGH_LIMIT:
            ...
            if (status) {
                if (Analog_High_Limit_Set(pObject,
                    value.type.Real)) {
                    ucix_add_option_float(ctxw, sec, idx_c, "high_limit", pObject->High_Limit, resolution);
                    ucix_commit(ctxw,sec);
                }
            }
            break;
        case PROP_LOW_LIMIT:
            ...
            if (status) {
                if (Analog_Low_Limit_Set(pObject,
                    value.type.Real)) {
                    ucix_add_option_float(ctxw, sec, idx_c, "low_limit", pObject->Low_Limit, resolution);
                    ucix_commit(ctxw,sec);
                }
            }
            break;
        case PROP_DEADBAND:
            ...
            if (status) {
                if (Analog_Deadband_Set(pObject,
                    ...
                    ucix_add_option_float(ctxw, sec, idx_c, "dead_limit", pObject->Deadband, resolution);
                    ucix_commit(ctxw,sec);
                }
            }
            break;
