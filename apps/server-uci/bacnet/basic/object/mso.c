...
static const char *sec = "bacnet_mo";
static const char *type = "mo";
...
static const int32_t Properties_Optional[] = {
    /* list of required properties in the object */
    PROP_STATE_TEXT, PROP_DESCRIPTION, PROP_RELIABILITY,
#if defined(INTRINSIC_REPORTING)
 ...
 * @return  status
 */
bool Multistate_Output_Max_States_Set(uint32_t object_instance,
    uint32_t count)
{
    bool status = false;
    struct object_data *pObject;
 ...
 */
uint32_t Multistate_Output_Feedback_Value(uint32_t object_instance)
{
    ...
    if (pObject) {
        if (pObject->Feedback_Value) {
            ...
            value = Multistate_Output_Present_Value(object_instance);
        }
    }
 ...
 */
uint32_t Multistate_Output_Time_Delay(uint32_t object_instance)
{
    uint32_t value = 0;
    struct object_data *pObject;
 ...
 */
bool Multistate_Output_Time_Delay_Set(uint32_t object_instance, uint32_t value)
{
    bool status = false;
    struct object_data *pObject;
 ...
 */
uint32_t Multistate_Output_Notification_Class(uint32_t object_instance)
{
    uint32_t value = 0;
    struct object_data *pObject;
 ...
 */
bool Multistate_Output_Notification_Class_Set(uint32_t object_instance, uint32_t value)
{
    bool status = false;
    struct object_data *pObject;
 ...
 */
bool Multistate_Output_Alarm_Value(uint32_t object_instance,
    uint32_t state)
{
    bool value = false;
    struct object_data *pObject;
 ...
 */
bool Multistate_Output_Alarm_Value_Set(uint32_t object_instance, 
 ...
 */
uint8_t Multistate_Output_Event_Enable(uint32_t object_instance)
{
    uint8_t value = 0;
    struct object_data *pObject;
 ...
 */
bool Multistate_Output_Event_Enable_Set(uint32_t object_instance, uint8_t value)
{
    bool status = false;
    struct object_data *pObject;
 ...
 * @return true
 */
bool Multistate_Output_Acked_Transitions(uint32_t object_instance, ACKED_INFO *value[MAX_BACNET_EVENT_TRANSITION])
{
    struct object_data *pObject;
    uint8_t b = 0;
 ...
 */
uint8_t Multistate_Output_Notify_Type(uint32_t object_instance)
{
    uint8_t value = 0;
    struct object_data *pObject;
...
#if 0
static void
Multistate_Output_Reset_Event_Properties(uint32_t object_instance)
{
    unsigned j;
    struct object_data *pObject;
 ...
 * @return true
 */
bool Multistate_Output_Event_Time_Stamps(uint32_t object_instance, BACNET_DATE_TIME *value[MAX_BACNET_EVENT_TRANSITION])
{
    struct object_data *pObject;
    uint8_t b = 0;
    pObject = Keylist_Data(Object_List, object_instance);
 ...
 */
static int Multistate_Output_Event_Message_Texts_Encode(
    uint32_t object_instance, BACNET_ARRAY_INDEX index, uint8_t *apdu)
{
    ...
    text = Multistate_Output_Event_Message_Text(object_instance, index);
    if (text) {
    ...
    BACNET_ERROR_CODE *error_code)
{
    ...
    pObject = Keylist_Data(Object_List, object_instance);
    if (pObject) {
        max_states = Multistate_Output_Max_States(object_instance);
 ...
 * @param  value - floating point analog value
 */
static void Multistate_Output_Present_Value_COV_Detect(
    ...
    uint32_t object_instance, uint32_t value, unsigned priority)
{
    ...
    if (pObject) {
        ...
        if ((value >= 1) && (value <= max_states) &&
            ...
            Multistate_Output_Present_Value_COV_Detect(
                pObject, Multistate_Output_Present_Value(object_instance));
            status = true;
        }
    }
 ...
 */
bool Multistate_Output_Overridden(uint32_t object_instance)
{
    bool value = false;
    struct object_data *pObject;
 ...
 */
void Multistate_Output_Overridden_Set(uint32_t object_instance, bool value)
{
    struct object_data *pObject;
 ...
 */
unsigned Multistate_Output_Event_State(uint32_t object_instance)
{
...
int Multistate_Output_Read_Property(BACNET_READ_PROPERTY_DATA *rpdata)
{
    ...
    switch (rpdata->object_property) {
        ...
        case PROP_FEEDBACK_VALUE:
            present_value = Multistate_Output_Feedback_Value(rpdata->object_instance);
            apdu_len = encode_application_unsigned(&apdu[0], present_value);
            break;
#endif
        ...
        case PROP_EVENT_STATE:
            ...
            apdu_len =
                encode_application_enumerated(&apdu[0], 
                    Multistate_Output_Event_State(rpdata->object_instance));
#else
            apdu_len =
        ...
        case PROP_TIME_DELAY:
            i = Multistate_Output_Time_Delay(rpdata->object_instance);
            apdu_len = encode_application_unsigned(&apdu[0], i);
            break;

        case PROP_NOTIFICATION_CLASS:
            i = Multistate_Output_Notification_Class(rpdata->object_instance);
            apdu_len = encode_application_unsigned(&apdu[0], i);
            break;
        ...
        case PROP_ALARM_VALUES:
            ...
            if (rpdata->array_index == 0) {
                ...
                /* into one packet. */
                for (i = 1; i <= max_states; i++) {
                    if (Multistate_Output_Alarm_Value(rpdata->object_instance, i)) {
        ...
        case PROP_EVENT_ENABLE:
            i = Multistate_Output_Event_Enable(rpdata->object_instance);
            bitstring_init(&bit_string);
        ...
        case PROP_ACKED_TRANSITIONS:
            state = Multistate_Output_Acked_Transitions(rpdata->object_instance, ack_info);
            bitstring_init(&bit_string);
            bitstring_set_bit(&bit_string, TRANSITION_TO_OFFNORMAL,
        ...
        case PROP_NOTIFY_TYPE:
            i = Multistate_Output_Notify_Type(rpdata->object_instance);
            apdu_len = encode_application_enumerated(
        ...
        case PROP_EVENT_TIME_STAMPS:
            ...
            /* into one packet. */
                ...
                state = Multistate_Output_Event_Time_Stamps(rpdata->object_instance, timestamp);
        ...
        case PROP_EVENT_MESSAGE_TEXTS:
            apdu_len = bacnet_array_encode(
                rpdata->object_instance, rpdata->array_index,
                Multistate_Output_Event_Message_Texts_Encode,
...
bool Multistate_Output_Write_Property(BACNET_WRITE_PROPERTY_DATA *wp_data)
{
    ...
    switch (wp_data->object_property) {
        case PROP_PRESENT_VALUE:
            ...
            if (status) {
                ...
                if (value.type.Unsigned_Int <= UINT32_MAX) {
                    if (Multistate_Output_Present_Value_Write(
                        wp_data->object_instance,
                        value.type.Unsigned_Int, wp_data->priority,
                        &wp_data->error_class, &wp_data->error_code)) {
                        value_i = Multistate_Output_Present_Value(wp_data->object_instance);
                ...
                if (status) {
                    if (Multistate_Output_Present_Value_Relinquish_Write(
        ...
        case PROP_STATE_TEXT:
            if (wp_data->array_index == 0) {
                ...
                for (idx = 1; idx <= 255; idx++ ) {
                    ...
                    if (element_len) {
                        status = Multistate_Output_State_Text_Set(
                ...
                if (idx != Multistate_Output_Max_States(wp_data->object_instance)) {
                    Multistate_Output_Max_States_Set(wp_data->object_instance, idx);
                }
            } else {
                status = write_property_type_valid(wp_data, &value,
                    BACNET_APPLICATION_TAG_CHARACTER_STRING);
        ...
        case PROP_NUMBER_OF_STATES:
            ...
            if (status)
                Multistate_Output_Max_States_Set(wp_data->object_instance,
        ...
        case PROP_RELIABILITY:
            status = write_property_type_valid(wp_data, &value,
                BACNET_APPLICATION_TAG_ENUMERATED);
            if (status)
                Multistate_Output_Reliability_Set(wp_data->object_instance,
                    value.type.Enumerated);
            break;
        case PROP_PRIORITY_ARRAY:
            wp_data->error_class = ERROR_CLASS_PROPERTY;
        ...
        case PROP_DESCRIPTION:
            status = write_property_type_valid(wp_data, &value,
                BACNET_APPLICATION_TAG_CHARACTER_STRING);
            if (status) {
                if (Multistate_Output_Description_Set(
                    wp_data->object_instance, value.type.Character_String.value)) {
                    ucix_add_option(ctxw, sec, idx_c, "description",
                        Multistate_Output_Description(wp_data->object_instance));
                    ucix_commit(ctxw,sec);
                }
            }
            break;
        ...
        case PROP_TIME_DELAY:
            ...
            if (status) {
                if (Multistate_Output_Time_Delay_Set(
                    wp_data->object_instance, value.type.Unsigned_Int)) {
                    ucix_add_option_int(ctxw, sec, idx_c, "time_delay",
                        Multistate_Output_Time_Delay(wp_data->object_instance));
                    ucix_commit(ctxw,sec);
                }
            }
            break;
        case PROP_NOTIFICATION_CLASS:
            status = write_property_type_valid(wp_data, &value,
                BACNET_APPLICATION_TAG_UNSIGNED_INT);
            if (status) {
                if (Multistate_Output_Notification_Class_Set(
                    wp_data->object_instance, value.type.Unsigned_Int)) {
                    ucix_add_option_int(ctxw, sec, idx_c, "nc",
                        Multistate_Output_Notification_Class(wp_data->object_instance));
                    ucix_commit(ctxw,sec);
                }
            }
            break;
        case PROP_ALARM_VALUES:
            if (wp_data->array_index == 0) {
                ...
                element_len = len;
                for (idx = 1; idx <= 255; idx++ ) {
                    Multistate_Output_Alarm_Value_Set(
                        wp_data->object_instance,
                        idx, false);
                }
                for (idx = 0; idx < 255; idx++ ) {
                    ...
                    if (element_len) {
                        status = Multistate_Output_Alarm_Value_Set(
                            wp_data->object_instance,
            ...
            if (status) {
                ...
                for (idx = 1 ; idx <= stats_n; idx++) {
                    if (Multistate_Output_Alarm_Value(wp_data->object_instance, idx)) {
        ...
        case PROP_NOTIFY_TYPE:
            status = write_property_type_valid(wp_data, &value,
                BACNET_APPLICATION_TAG_ENUMERATED);
            if (status) {
                if (Multistate_Output_Notify_Type_Set(
                    wp_data->object_instance, value.type.Enumerated)) {
                    ucix_add_option_int(ctxw, sec, idx_c, "notify_type",
                        Multistate_Output_Notify_Type(wp_data->object_instance));
                    ucix_commit(ctxw,sec);
                    }
            }
            break;
...
#if defined(INTRINSIC_REPORTING)
static const char *Multistate_Output_Event_Message(
    uint32_t object_instance,
    ...
    uint32_t object_instance)
{
    ...
    if (pObject->Ack_notify_data.bSendAckNotify) {
        ...
        debug_printf(
            "Multi-State-Output[%d]: Send Acknotification.\n", object_instance);
        ...
        if (FromState != ToState ||
            ...
            debug_printf(
                "Multi-State-Output[%d]: Event_State goes from %s to %s.\n",
    ...
    if (SendNotify) {
        ...
        if (event_data.notifyType != NOTIFY_ACK_NOTIFICATION) {
            if (event_data.eventType == EVENT_OUT_OF_RANGE) {
                ...
                event_data.notificationParams.changeOfState.newState.state.unsignedValue = pObject->Prior_Value;
        ...
        debug_printf(
            "Multi-State-Output[%d]: Notification Class[%d]-%s "
            "%u/%u/%u-%u:%u:%u.%u!\n",
        ...
        if ((event_data.notifyType != NOTIFY_ACK_NOTIFICATION) &&
            ...
            debug_printf("Multi-State-Output[%d]: Ack Required!\n", object_instance);
    ...
    BACNET_GET_EVENT_INFORMATION_DATA * getevent_data)
{
    ...
    pObject = Keylist_Data(Object_List, Multistate_Output_Index_To_Instance(index));
...
#if defined(INTRINSIC_REPORTING)
    value_i = ucix_get_option_int(ictx->ctx, ictx->section, sec_idx, "fb_value", 0);
    pObject->Feedback_Value = value_i;
#endif
#if defined(INTRINSIC_REPORTING)
    pObject->Event_State = EVENT_STATE_NORMAL;
...
void Multistate_Output_Init(void)
{
    ...
    handler_alarm_ack_set(Object_Type, Multistate_Output_Alarm_Ack);
