...
static void PrintHeading(void)
{
    ...
    printf("{\n");
    printf("\"PICS\":0,\n");
    printf("\"description\":\"BACnet Protocol Implementation Conformance Statement\",\n");
    printf("\"info\":\"Generated by BACnet Protocol Stack library EPICS tool ");
    ...
    if ((value != NULL) &&
        ...
        printf("\"Vendor_Name\":\"your vendor name here\",\n");
    }
    ...
    if ((value != NULL) &&
        ...
        printf(
            ...
            characterstring_value(&value->type.Character_String));
        printf(
            "\"Product_Model_Number\":\"%s\",\n",
            characterstring_value(&value->type.Character_String));
    } else {
        printf("\"Product_Name\":\"your product name here\",\n");
        printf("\"Product_Model_Number\":\"your model number here\",\n");
    }
    ...
    if ((value != NULL) && (value->tag == BACNET_APPLICATION_TAG_BIT_STRING)) {
        ...
        printf("\"services_reported\":[\n");
        for (i = 0; i < len; i++) {
            if (bitstring_bit(&value->type.Bit_String, (uint8_t)i)) {
                if ( i > 0) {
                    printf(",\n");
                }
                printf("\"%s\"", protocol_services_supported_text(i));
            }
        }
        printf("\n],\n");
    } else {
    ...
    if ((value != NULL) && (value->tag == BACNET_APPLICATION_TAG_BIT_STRING)) {
        int i, len = bitstring_bits_used(&value->type.Bit_String);
        printf("\"objects_reported\":[\n");
        for (i = 0; i < len; i++) {
            if (bitstring_bit(&value->type.Bit_String, (uint8_t)i)) {
                if ( i > 0) {
                    printf(",\n");
                }
                printf("\"%s\"", bactext_object_type_name(i));
            }
        }
        printf("\n],\n");
    } else {
